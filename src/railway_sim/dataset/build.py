...
#:
#: 區間車與區間快是例外：五型通勤電聯車（EMU500、600、700、800、900）在同一
#: 段路線上**共通運用**，時刻表也沒有印是哪一型，因此「這一班開哪一型」根本
#: 不是匯入時決定得了的事。這裡填的 EMU900 在執行時只當作「本班次屬於共通
#: 運用池」的標記，真正派哪一型由 ``trains.json`` 的 ``rolling_stock_pools``
#: 逐日抽籤（見 :mod:`railway_sim.timetable.rolling_stock`）。
...
#: 屬於共通運用池，由 ``rolling_stock_pools`` 的六家線規則派 EMU500 或
#: EMU600。
