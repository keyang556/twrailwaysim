...
def _run_import(args: argparse.Namespace, data_dir: Path) -> int:
    source_map_path = data_dir / SOURCE_MAP_FILENAME
    try:
...
def _run_check(data_dir: Path) -> int:
    data = load_game_data(data_dir)
